...
//     const url = `${currentUrl}?filter=&promoProduct=0&pattern=${pattern}&occasion=${ocassion}&size=${size}&color=${color}&fabric=${fabric}&priceRange=${newValue}&order=${category}&featured=${
//       featured ? featured : ""
...
//     <>
//       {type === "mobile" ? (
//         <AccordionLayout title="price range" id="price-id" open={true}>
//           <div className="row">
...
export default function PriceFilter({
  ...
  // =============== state ================
  // Safeguard for max rate, fallback to 10000 if maxRate is not available
  const initialMax = parseInt(data?.maxRate) || 10000;
  const [value, setValue] = useState([1, initialMax]);
  ...
  useEffect(() => {
    const parsedMax = parseInt(data?.maxRate);
    if (!isNaN(parsedMax)) {
      const newVal = [1, parsedMax];
      setValue(newVal);
      setComittedValue(newVal);
