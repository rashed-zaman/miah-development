...
import { useRouter } from "next/navigation"; // <-- Next 15 router
...
import { addToBag } from "@/store/shoppingBagSlice";
...
export default function AddToBag({ product, selectedVariation, sku, size }) {
  ...
  useEffect(() => {
    setItemQty(1);
    setOutofStock(false);
  }, [router.pathname, selectedVariation]); // <-- updated from asPath to pathname for Next 15
  ...
  return (
    <>
      {product.category_id === 7 ? (
        <div>
          <Grid container>
            <Grid size={{xs:7, sm:12}} sx={{ textAlign: "left" }}>
